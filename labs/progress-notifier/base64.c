...
{
    infof("THIS IS THE PROGRESS: %ld, AND THE TOTAL: %ld,  %%: %ld%%\n", progress, total, progress*100/total);
}
...
int handleFile(char *inputFile, char *outputFile, int * handler(void *, long , char *, long *)){
    ...
    if((file = fopen(inputFile,"r"))==NULL){
        errorf("The file cannot be openned\n");
        return 0;
    }
...
int main(int argc, char** argv)
{
    ...
    if(argc!=3){
        warnf("There are missing some parameters. Check again.\n");
        return 0;
    }
    if(strcmp(argv[1], "--encode")==0) {
        ...
        handleFile(argv[2], newName2, base64encode);
    } 
        ...
        handleFile(argv[2], newName2, base64decode);
    } 
    else {
        errorf("Something was wrong\n");
        return 0;
    }
    return 0;
}